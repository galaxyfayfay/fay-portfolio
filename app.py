...
def build_gallery(photos):
    ...
    slots = "".join([
        f'<div class="photo-slot"><img src="{p["src"]}" alt="{p["name"]}"></div>'
        for p in photos[:6]
    ])
...
# ── Inject full portfolio as HTML via st.markdown ──
st.markdown(f"""
<style>
...
[data-testid="stAppViewContainer"] {{
  padding: 0 !important; background: #0e1912 !important;
}}
[data-testid="stVerticalBlock"] {{ gap: 0 !important; padding: 0 !important; }}

/* Portfolio styles — all scoped under #fay-root */
...
:root {{
  --ink:#0e1912; --deep:#172a1d; --forest:#1f3625;
  --gold:#c9a96e; --gold-light:#dfc28a;
  --cream:#f3ead9; --mist:#b8c8b2;
}}
...
#fay-ring.on {{ width: 48px; height: 48px; border-color: var(--gold); }}

/* Orbs */
.fay-orb {{ position: fixed; border-radius: 50%; filter: blur(72px); pointer-events: none; }}
.fay-orb-1 {{
  width: 380px; height: 380px; background: rgba(50,85,60,.11);
  top: -8%; right: -4%; z-index: 0;
  animation: fay-orb 14s ease-in-out infinite;
}}
.fay-orb-2 {{
  width: 280px; height: 280px; background: rgba(201,169,110,.045);
  bottom: 18%; left: -7%; z-index: 0;
...
.fay-logo {{
  ...
  font-size: 1rem; letter-spacing: .18em; color: var(--gold);
  text-decoration: none; font-weight: 300;
}}
.fay-nav-links {{ display: flex; gap: 36px; list-style: none; margin: 0; padding: 0; }}
.fay-nav-links a {{
  color: var(--mist); text-decoration: none;
  font-size: .7rem; letter-spacing: .22em; text-transform: uppercase;
  font-family: 'EB Garamond', Georgia, serif; transition: color .25s;
}}
...
.fay-hero-bg::before {{
  content: ''; position: absolute; right: 9%; top: 12%; width: 1px; height: 68%;
...
.fay-hero-bg::after {{
  content: ''; position: absolute; right: 14%; top: 22%; width: 1px; height: 46%;
...
@keyframes fay-breathe {{ 0%,100%{{ opacity:.4 }} 50%{{ opacity:1 }} }}

.fay-hero-content {{ position: relative; z-index: 1; max-width: 880px; }}
.fay-eyebrow {{
  font-family: 'EB Garamond', Georgia, serif;
  font-size: .68rem; letter-spacing: .38em; text-transform: uppercase;
  color: var(--gold); margin-bottom: 28px;
  display: flex; align-items: center; gap: 14px;
  opacity: 0; animation: fay-up 1s .3s ease forwards;
}}
.fay-eyebrow::before {{ content: ''; width: 36px; height: 1px; background: var(--gold); flex-shrink: 0; }}
.fay-name-zh {{
  font-family: 'Noto Serif SC', Georgia, serif;
  font-size: clamp(1.1rem, 2.2vw, 1.6rem); font-weight: 300; letter-spacing: .14em;
  color: rgba(243,234,217,.4);
  opacity: 0; animation: fay-up 1.2s .5s ease forwards; margin-bottom: 4px;
}}
.fay-name-en {{
  font-family: 'Cormorant Garamond', Georgia, serif;
  font-size: clamp(4.5rem, 10vw, 9rem); font-weight: 300; font-style: italic;
  color: var(--cream); line-height: .9;
  opacity: 0; animation: fay-up 1.2s .65s ease forwards; margin-bottom: 44px;
}}
.fay-desc {{
  font-size: .98rem; line-height: 2; color: rgba(243,234,217,.55);
  font-weight: 300; max-width: 480px;
  opacity: 0; animation: fay-up 1.2s .85s ease forwards; margin-bottom: 48px;
}}
.fay-tags {{
  display: flex; gap: 10px; flex-wrap: wrap;
  opacity: 0; animation: fay-up 1.2s 1.05s ease forwards;
}}
.fay-tag {{
  padding: 6px 16px; border: 1px solid rgba(201,169,110,.28);
  font-size: .68rem; letter-spacing: .14em; text-transform: uppercase;
  color: var(--gold); font-family: 'EB Garamond', serif; transition: all .3s;
}}
.fay-tag:hover {{ background: rgba(201,169,110,.07); border-color: var(--gold); }}
.fay-scroll-hint {{
  position: absolute; bottom: 38px; left: 60px;
  display: flex; align-items: center; gap: 12px;
  color: rgba(243,234,217,.28); font-size: .62rem; letter-spacing: .28em;
  text-transform: uppercase; font-family: 'EB Garamond', serif;
  opacity: 0; animation: fay-up 1s 1.5s ease forwards;
}}
.fay-scroll-line {{ width: 52px; height: 1px; background: rgba(243,234,217,.15); position: relative; overflow: hidden; }}
.fay-scroll-line::after {{
  content: ''; position: absolute; top: 0; left: -100%; width: 100%; height: 100%;
  background: var(--gold); animation: fay-slide 2.2s ease-in-out infinite;
}}
@keyframes fay-slide {{ 0%{{ left:-100% }} 100%{{ left:100% }} }}
@keyframes fay-up {{
  from {{ opacity:0; transform:translateY(22px); }}
  to {{ opacity:1; transform:translateY(0); }}
}}

.fay-divider {{ height: 1px; background: linear-gradient(to right, transparent, rgba(201,169,110,.18), transparent); }}

.fay-label {{
  font-family: 'EB Garamond', Georgia, serif;
  font-size: .66rem; letter-spacing: .38em; text-transform: uppercase;
  color: var(--gold); display: flex; align-items: center; gap: 14px; margin-bottom: 60px;
}}
.fay-label::before {{ content: ''; width: 28px; height: 1px; background: var(--gold); flex-shrink: 0; }}

/* About */
#fay-about {{ padding: 130px 60px; background: linear-gradient(to bottom, var(--ink), var(--deep)); }}
.fay-about-grid {{ display: grid; grid-template-columns: 1fr 1.2fr; gap: 90px; align-items: start; }}
.fay-photo-frame {{ position: relative; }}
.fay-photo-wrap {{
  width: 100%; aspect-ratio: 3/4; background: var(--forest); overflow: hidden; position: relative;
}}
.fay-photo-wrap::before {{
  content: 'Yu'; position: absolute; inset: 0;
  display: flex; align-items: center; justify-content: center;
  font-family: 'Cormorant Garamond', serif;
  font-size: 7rem; font-weight: 300; color: rgba(201,169,110,.08);
}}
.fay-photo-wrap img {{ width: 100%; height: 100%; object-fit: cover; display: block; position: relative; z-index: 1; }}
...
.fay-about-text h2 {{
  ...
  line-height: 1.15; color: var(--cream); margin-bottom: 32px;
}}
.fay-about-text h2 em {{ font-style: italic; color: var(--gold-light); }}
.fay-about-text p {{ font-size: .95rem; line-height: 2.1; color: rgba(243,234,217,.58); font-weight: 300; margin-bottom: 18px; }}
.fay-stats {{
  display: grid; grid-template-columns: 1fr 1fr; gap: 28px;
  margin-top: 48px; padding-top: 44px; border-top: 1px solid rgba(201,169,110,.12);
}}
.fay-stat-num {{
  font-family: 'Cormorant Garamond', serif;
  font-size: 2.5rem; font-weight: 300; color: var(--gold-light); line-height: 1; margin-bottom: 6px;
}}
.fay-stat-label {{
...
/* Experience */
#fay-exp {{ padding: 130px 60px; background: var(--deep); }}
.fay-timeline {{ position: relative; padding-left: 38px; }}
.fay-timeline::before {{
...
.fay-exp-dot {{
  position: absolute; left: -42px; top: 9px; width: 8px; height: 8px;
  border: 1px solid var(--gold); border-radius: 50%; background: var(--deep); transition: all .3s;
}}
.fay-exp-item:hover .fay-exp-dot {{ background: var(--gold); box-shadow: 0 0 10px rgba(201,169,110,.4); }}
.fay-exp-meta {{ display: flex; justify-content: space-between; align-items: baseline; margin-bottom: 6px; flex-wrap: wrap; gap: 6px; }}
.fay-exp-co {{ font-family: 'Noto Serif SC', serif; font-size: 1.05rem; color: var(--cream); font-weight: 400; }}
.fay-exp-date {{ font-family: 'EB Garamond', serif; font-size: .78rem; color: var(--gold); letter-spacing: .1em; }}
.fay-exp-role {{ font-family: 'EB Garamond', serif; font-size: .82rem; color: var(--gold-light); font-style: italic; margin-bottom: 18px; }}
.fay-exp-hls {{ display: flex; flex-direction: column; gap: 10px; }}
.fay-hl {{
  display: flex; gap: 12px; font-size: .84rem;
  color: rgba(243,234,217,.52); font-family: 'Noto Serif SC', serif;
  line-height: 1.85; font-weight: 300;
}}
.fay-hl::before {{ content: '◆'; font-size: .38rem; color: var(--gold); margin-top: 8px; flex-shrink: 0; }}
...
#fay-edu::before {{
  content: 'EDU'; position: absolute; right: -1%; top: 50%; transform: translateY(-50%);
  font-family: 'Cormorant Garamond', serif; font-size: 19vw; font-weight: 300;
  color: rgba(201,169,110,.022); pointer-events: none;
}}
.fay-edu-cards {{ display: grid; grid-template-columns: 1fr 1fr; gap: 28px; }}
.fay-edu-card {{
  border: 1px solid rgba(201,169,110,.13); padding: 44px;
...
.fay-edu-card:hover {{ border-color: rgba(201,169,110,.38); background: rgba(201,169,110,.025); }}
.fay-edu-badge {{ font-family: 'Cormorant Garamond', serif; font-size: 2.8rem; font-weight: 300; color: rgba(201,169,110,.18); line-height: 1; margin-bottom: 22px; }}
.fay-edu-school {{ font-family: 'Noto Serif SC', serif; font-size: 1.1rem; color: var(--cream); margin-bottom: 6px; }}
.fay-edu-degree {{ font-family: 'EB Garamond', serif; font-size: .82rem; color: var(--gold-light); font-style: italic; margin-bottom: 18px; }}
.fay-edu-period {{ font-family: 'EB Garamond', serif; font-size: .68rem; letter-spacing: .2em; color: rgba(243,234,217,.28); text-transform: uppercase; margin-bottom: 20px; }}
.fay-edu-detail {{ font-size: .8rem; line-height: 1.9; color: rgba(243,234,217,.42); font-family: 'Noto Serif SC', serif; font-weight: 300; }}

/* Projects */
#fay-proj {{ padding: 130px 60px; background: var(--deep); }}
.fay-proj-grid {{ display: grid; grid-template-columns: 1.35fr 1fr; gap: 22px; }}
.fay-proj-card {{
  position: relative; overflow: hidden;
...
.fay-proj-inner {{
  height: 100%; min-height: 300px; position: relative; padding: 48px 44px;
  border: 1px solid rgba(201,169,110,.1); transition: border-color .45s;
...
.fay-proj-bg {{ position: absolute; top: 22px; right: 22px; font-family: 'Cormorant Garamond', serif; font-size: 4.5rem; font-weight: 300; color: rgba(201,169,110,.07); line-height: 1; }}
.fay-proj-content {{ position: relative; z-index: 1; }}
.fay-proj-num {{ font-family: 'Cormorant Garamond', serif; font-size: .76rem; color: var(--gold); letter-spacing: .16em; margin-bottom: 14px; }}
.fay-proj-title {{ font-family: 'Noto Serif SC', serif; font-size: 1.2rem; color: var(--cream); margin-bottom: 10px; font-weight: 400; line-height: 1.4; }}
.fay-proj-desc {{ font-size: .8rem; color: rgba(243,234,217,.48); line-height: 1.88; font-family: 'Noto Serif SC', serif; font-weight: 300; margin-bottom: 22px; }}
.fay-proj-link {{
  display: inline-flex; align-items: center; gap: 8px;
  font-family: 'EB Garamond', serif; font-size: .72rem; letter-spacing: .2em;
  text-transform: uppercase; color: var(--gold); text-decoration: none; transition: gap .3s;
}}
.fay-proj-link:hover {{ gap: 14px; }}
.fay-proj-link::after {{ content: '→'; }}

/* Photography */
#fay-photo {{ padding: 130px 60px; background: var(--ink); }}
.fay-photo-intro {{ max-width: 520px; margin-bottom: 72px; }}
.fay-photo-intro p {{ font-size: .95rem; line-height: 2.05; color: rgba(243,234,217,.48); font-weight: 300; }}
.fay-gallery {{ display: grid; grid-template-columns: repeat(3, 1fr); gap: 3px; }}
.photo-slot {{
  aspect-ratio: 4/5; background: var(--forest);
  position: relative; overflow: hidden;
  border: 1px solid rgba(201,169,110,.07); transition: border-color .45s; cursor: pointer;
}}
...
.photo-slot img {{ width: 100%; height: 100%; object-fit: cover; display: block; transition: transform .75s cubic-bezier(.25,.46,.45,.94); position: relative; z-index: 1; }}
.photo-slot:hover img {{ transform: scale(1.04); }}

/* Skills */
#fay-skills {{ padding: 130px 60px; background: linear-gradient(180deg, var(--deep), var(--ink)); }}
.fay-skills-layout {{ display: grid; grid-template-columns: 1fr 1fr; gap: 76px; align-items: start; }}
.fay-skill-group {{ margin-bottom: 44px; }}
.fay-skill-group-title {{
  font-family: 'EB Garamond', serif; font-size: .67rem; letter-spacing: .28em;
  text-transform: uppercase; color: var(--gold); margin-bottom: 22px;
  padding-bottom: 10px; border-bottom: 1px solid rgba(201,169,110,.13);
}}
.fay-skill-items {{ display: flex; flex-wrap: wrap; gap: 7px; }}
.fay-skill-item {{
  padding: 6px 16px; font-size: .76rem; font-family: 'Noto Serif SC', serif;
  color: rgba(243,234,217,.55); border: 1px solid rgba(243,234,217,.1);
  transition: all .28s; font-weight: 300;
}}
.fay-skill-item:hover {{ color: var(--gold-light); border-color: rgba(201,169,110,.38); background: rgba(201,169,110,.04); }}
.fay-lang-list {{ display: flex; flex-direction: column; gap: 18px; }}
.fay-lang-name {{ display: flex; justify-content: space-between; margin-bottom: 7px; font-size: .8rem; color: rgba(243,234,217,.65); font-family: 'Noto Serif SC', serif; }}
.fay-lang-bar {{ height: 1px; background: rgba(243,234,217,.07); position: relative; overflow: hidden; }}
.fay-lang-fill {{
...
/* Contact */
#fay-contact {{ padding: 130px 60px 72px; background: var(--ink); }}
.fay-contact-ghost {{
  font-family: 'Cormorant Garamond', serif;
  font-size: clamp(3rem, 8vw, 7rem); font-weight: 300;
  color: rgba(243,234,217,.055); margin-bottom: 72px; line-height: 1; pointer-events: none;
}}
.fay-contact-grid {{ display: grid; grid-template-columns: 1fr 1fr; gap: 72px; align-items: end; }}
.fay-contact-left h3 {{ font-family: 'Cormorant Garamond', serif; font-size: 2.3rem; font-weight: 300; color: var(--cream); margin-bottom: 18px; }}
.fay-contact-left p {{ font-size: .9rem; line-height: 1.95; color: rgba(243,234,217,.42); font-family: 'Noto Serif SC', serif; font-weight: 300; max-width: 380px; }}
.fay-contact-links {{ display: flex; flex-direction: column; gap: 22px; align-items: flex-end; }}
.fay-contact-link {{ display: flex; align-items: center; gap: 12px; color: rgba(243,234,217,.45); text-decoration: none; font-family: 'EB Garamond', serif; font-size: .96rem; transition: color .28s; }}
.fay-contact-link:hover {{ color: var(--gold-light); }}
.fay-contact-link span {{ font-size: .62rem; letter-spacing: .22em; text-transform: uppercase; color: var(--gold); opacity: .68; }}
.fay-footer {{
...
.fay-footer-sig {{ font-family: 'Cormorant Garamond', serif; font-size: .95rem; font-style: italic; color: rgba(243,234,217,.18); }}
.fay-footer-copy {{ font-size: .64rem; letter-spacing: .14em; color: rgba(243,234,217,.14); font-family: 'EB Garamond', serif; text-transform: uppercase; }}
...
@media (max-width: 860px) {{
  ...
  .fay-skills-layout, .fay-contact-grid {{ grid-template-columns: 1fr; gap: 44px; }}
  ...
  <div class="fay-about-grid">
    <div class="fay-photo-frame">
      <div class="fay-photo-wrap">
        <!-- portrait: put photos/portrait.jpg in the photos/ folder -->
      </div>
    </div>
    ...
    <div>
      <div class="fay-skill-group">
        ...
        <div class="fay-skill-items">
          <span class="fay-skill-item">Python</span><span class="fay-skill-item">SQL</span>
          <span class="fay-skill-item">Stata</span><span class="fay-skill-item">Figma</span>
          <span class="fay-skill-item">XMind</span><span class="fay-skill-item">Visio</span>
          <span class="fay-skill-item">Microsoft Office</span><span class="fay-skill-item">Vibe Coding</span>
      ...
      <div class="fay-skill-group">
        ...
        <div class="fay-skill-items">
          <span class="fay-skill-item">用户研究</span><span class="fay-skill-item">A/B 实验设计</span>
          <span class="fay-skill-item">数据分析</span><span class="fay-skill-item">产品运营</span>
          <span class="fay-skill-item">增长策略</span><span class="fay-skill-item">需求文档</span>
    ...
    <div>
      <div class="fay-skill-group">
        ...
        <div class="fay-lang-list">
          <div><div class="fay-lang-name"><span>普通话</span><span>母语</span></div><div class="fay-lang-bar"><div class="fay-lang-fill" style="--pct:100%"></div></div></div>
          <div><div class="fay-lang-name"><span>粤语</span><span>日常沟通</span></div><div class="fay-lang-bar"><div class="fay-lang-fill" style="--pct:65%"></div></div></div>
          <div><div class="fay-lang-name"><span>English</span><span>IELTS 7.5 · CET-6 584</span></div><div class="fay-lang-bar"><div class="fay-lang-fill" style="--pct:88%"></div></div></div>
          <div><div class="fay-lang-name"><span>Deutsch</span><span>基础</span></div><div class="fay-lang-bar"><div class="fay-lang-fill" style="--pct:28%"></div></div></div>
        </div>
      </div>
    </div>
  </div>
...
(function() {{
  ...
  var skillsEl = document.getElementById('fay-skills');
  if (skillsEl) {{
